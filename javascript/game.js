...
function start() {
    for (const light of lights) {
        ...
        light.classList.remove('jumpstart');
    ...
    function frame(now) {
        ...
        if (toLight < 5) {
            ...
            timeout = setTimeout(() => {
                ...
                for (const lightStrip of lights) {
                    lightStrip.classList.add('green');
                }
            }, delay);
        }
    }
...
function tap(event) {
    ...
    if (event.target.id === 'ranking-button') {
